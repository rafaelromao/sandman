...
- OpenCode preset now snapshots `~/.config/opencode/` and the non-mutable parts of `~/.local/share/opencode/`, but skips seven mutable subdirectories (`token-optimizer`, `storage`, `snapshot`, `tool-output`, `repos`, `log`, `node_modules`) and bind-mounts the SQLite session database (`opencode.db`, `opencode.db-shm`, `opencode.db-wal`) live. Host-side OpenCode sessions remain inspectable after a container run completes, and the run-owned snapshot no longer carries hundreds of MB of duplicated mutable state (ADR-0016).

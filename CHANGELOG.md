...
- `sandman tdd` skill now reuses an existing `## Plan` section in `.sandman/task.md` instead of drafting a new plan. The skill file opens with an `## About the plan` explanatory section that describes plan reuse before the workflow, and the planning step branches on plan presence: when a plan exists the workflow skips drafting and skips the plan-review subagent; when no plan exists, the original deep-modules / testability / subagent-review flow runs unchanged. The plan output shape (documented in the `sandman-plan` SKILL.md `Plan output shape` block) drops the `### Next step` subsection. `prompt.ParseTask` exposes the parsed `## Plan` body as `TaskDoc.Plan` and keeps the plan out of `TaskDoc.Body` so the resume prompt does not duplicate the plan. Note: the `## Next Step` heading in the task.md scaffold is part of the handoff/resume flow (per ADRs 0022/0023) and remains in `EmptyTaskTemplate` and `BuildTaskPrompt` — the "no Next Step" change is scoped to the `## Plan` section only. Verification: Go unit tests cover the parser, and the SKILL.md string-content guards (`TestParseTask_PlanSectionPresent/Missing/Empty`, `TestSandmanTddSkill_PlanReuseAndNoPlanBranches`, `TestSandmanPlanSkill_OutputShapeNoNextStep`). (#912)

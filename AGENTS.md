...
- **Required status checks on `main`:** `CI / build` (matrix `ubuntu-latest` + `macos-latest`, `pull_request` and `push` triggers from `.github/workflows/go.yml`), `CI / semantic-pull-request`, `Full Regression - Linux / Full Regression Suite (Ubuntu)`, and `Full Regression - macOS / Full Regression Suite (macOS)`. The full regression gates run their complete test tiers on every change request and `workflow_dispatch`; a successful report always includes non-zero executed-test counts and elapsed durations for each non-skipped tier. All checks must pass before the GitHub merge button enables.

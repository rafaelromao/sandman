...
- **Required status checks on `main`:** `CI / build` (matrix `ubuntu-latest` + `macos-latest`, `pull_request` and `push` triggers from `.github/workflows/go.yml`), `CI / semantic-pull-request`, `Full Regression - Linux / Full Regression Suite (Ubuntu)`, and `Full Regression - macOS / Full Regression Suite (macOS)`. The full regression gates run the real suite only on release-please release PRs (and on `workflow_dispatch`); on other change requests they report success immediately so normal merges are not blocked. All checks must pass before the GitHub merge button enables.

...
- **Command sockets**: `<batch>/run.sock` is the batch-lifetime command route for per-issue aborts, while `<batch>/runs/<runID>/run.sock` is attempt-local. Both use the same JSON command protocol; `batch.sock` remains attach-streaming only.
...
- **SemVer is derived from merged Conventional Commits.** Release Please aggregates the merged commit type stack: `feat:` → minor bump, `fix:` / `perf:` → patch, `feat!:` / `fix!:` / `perf!:` → major. Everything else is changelog-only. The pull-request title is validated separately by CI; title validation does not replace Release Please's commit-history parsing. Release Please opens a Release change request that applies the next SemVer tag. With the initial RC published, Release Please calculates subsequent RC versions automatically from release-bearing merged history.
- **Required status checks on `main`:** `CI / build` (matrix `ubuntu-latest` + `macos-latest`, `pull_request` and `push` triggers from `.github/workflows/go.yml`) and `CI / semantic-pull-request`. Both must pass before the GitHub merge button enables. The exhaustive Linux regression (`Full Regression - Linux`) and focused `Native Compatibility` workflows run on pushes to `release-please--branches--main` (and on `workflow_dispatch`); their failures are visible on the release PR but require maintainer review rather than a static ruleset check. The platform-coverage contract is enforced by `TestPlatformCoverageMapMatchesWorkflowsAndGoReleaser` in `scripts/release_workflow_test.go` and matches the four-platform matrix in `docs/development/testing.md`.
...
- **Issue branch base** — issue branches named `<issue-no>-<slug>` (the runtime's worktree branch name) are cut from the feature branch, not from `main`. The Sandman Go runtime resolves the base via `git.base_branch`; override the default `main` with `sandman run --base-branch <feature>` (or `.sandman/config.yaml:git.base_branch`) when running an issue that belongs to a feature-branch initiative. The branch name does not encode the feature branch (git's ref-namespace constraints prevent it); the feature-branch relationship is the source branch on the worktree cut, not the branch name.

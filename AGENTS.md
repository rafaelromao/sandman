...
- **SemVer is derived from merged Conventional Commits.** Release Please aggregates the merged commit type stack: `feat:` → minor bump, `fix:` / `perf:` → patch, `feat!:` / `fix!:` / `perf!:` → major. Everything else is changelog-only. The pull-request title is validated separately by CI; title validation does not replace Release Please's commit-history parsing. Release Please opens a Release change request that applies the next SemVer tag. The first-release bootstrap is the one-time exception: `release-as: 1.0.0` forces the initial release and must be removed after tag `v1.0.0` is created.

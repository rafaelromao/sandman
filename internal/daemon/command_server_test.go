...
func TestCommandServer_RejectsMissingIssueForBoundRun(t *testing.T) {
	dir := testenv.MkdirShort(t, "sm-cmd-")
	stub := &fakeCommander{}
	server := NewCommandServerForIssue(dir, stub, 42)
	if err := server.Start(); err != nil {
	...
	if err := json.NewEncoder(conn).Encode(CommandRequest{Action: "abort"}); err != nil {
		t.Fatalf("encode request: %v", err)
	}
...
func TestCommandServer_RejectsAbortForZeroBoundRun(t *testing.T) {
	dir := testenv.MkdirShort(t, "sm-cmd-")
	stub := &fakeCommander{}
	server := NewCommandServerForIssue(dir, stub, 0)
	if err := server.Start(); err != nil {
		t.Fatalf("Start failed: %v", err)
	}
	defer server.Stop()
	...
	if err := json.NewEncoder(conn).Encode(CommandRequest{Action: "abort", Issue: 0}); err != nil {
		t.Fatalf("encode request: %v", err)
	}

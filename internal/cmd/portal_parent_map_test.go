...
func TestPortal_Compute_ReasonTableForAllRunKinds(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			events: []events.Event{
				{Type: "run.started", Timestamp: time.Date(2025, 1, 1, 12, 0, 0, 0, time.UTC), RunID: "260618113825-abcd-42", Issue: 42, Payload: map[string]any{"branch": "42-fix"}},
				{Type: "run.finished", Timestamp: time.Date(2025, 1, 1, 12, 5, 0, 0, time.UTC), RunID: "260618113825-abcd-42", Issue: 42, Payload: map[string]any{"status": "success", "branch": "42-fix"}},
			},
...
func TestPortal_Compute_AggregatesChildReviewsOntoIssueRow(t *testing.T) {
	...
	if issueRow.Status != "success" {
		t.Fatalf("expected parent status success (terminal parent status preserved per #1825 contract; live review child does not flip terminal badge), got %q", issueRow.Status)
	}
...
func TestPortal_Compute_PreservesParentStatusWithTrailingQuoteRunLogShape(t *testing.T) {
	...
	writePortalLog(t, filepath.Join(repoRoot, ".sandman", "events.jsonl"), []events.Event{
		{Type: "run.started", Timestamp: startedAt, RunID: implRunID, Issue: 1755, Payload: map[string]any{
			"branch":      "1763-embed-sandman-index-as-sub-skill",
...
func TestPortal_Compute_PreservesParentStatusWithTrailingQuoteAndPipeRunLogShape(t *testing.T) {
	...
	writePortalLog(t, filepath.Join(repoRoot, ".sandman", "events.jsonl"), []events.Event{
		{Type: "run.started", Timestamp: startedAt, RunID: implRunID, Issue: 1779, Payload: map[string]any{
			"branch":      "1789-slice-0-e2e-gate-hygiene",
			"batch_id":    implBatchID,
			"base_branch": "main",
		}},
...
func TestPortal_Compute_CanonicalParentIdentityPreservedWithReviewChildren(t *testing.T) {
	...
	if parentRow.Status != "success" {
		t.Fatalf("expected canonical parent Status success (terminal parent status preserved per #1825 contract; live review child does not flip terminal badge), got %q", parentRow.Status)
	}
...
// TestPortal_ParentSuccWithLiveChild_PreservesTerminalStatus pins the #1825
// contract: a terminal parent impl row (success) with a live (active) sibling
// review child must preserve status "success" — no badge-flip to "reviewing".
// This matches the e2e regression test for the #1825 live-run atomicity
// invariant. aggregateReviewChildren no longer flips terminal parent status.
func TestPortal_ParentSuccWithLiveChild_PreservesTerminalStatus(t *testing.T) {
	...
	if issueRow.Status != "success" {
		t.Fatalf("Status = %q, want %q (terminal parent status preserved per #1825 contract)", issueRow.Status, "success")
	}
...
func TestPortal_KindForRun_BlockedStateReturnsCompleted(t *testing.T) {
	...
	runState := events.RunState{
		...
		Started:  events.Event{Timestamp: startedAt, Payload: map[string]any{"branch": "42-fix"}},
...
func TestPortal_Compute_CompletedRunWithDeadBatchDir_ReportsSourceExists(t *testing.T) {
	...
	writePortalLog(t, filepath.Join(repoRoot, ".sandman", "events.jsonl"), []events.Event{
		{Type: "run.started", Timestamp: startedAt, RunID: runID, Issue: 42, Payload: map[string]any{"branch": "42-fix", "batch_id": batchName}},
		{Type: "run.finished", Timestamp: finishedAt, RunID: runID, Issue: 42, Payload: map[string]any{"status": "success", "branch": "42-fix"}},
	})
...
func TestPortal_Compute_OrphanedActiveRunFromDeadBatch_Demoted(t *testing.T) {
	...
	runManifest := map[string]any{"run_id": runID, "issue": 42, "branch": "42-fix"}
...
func TestPortal_Compute_LiveParentAndDeadReviewChild_DoesNotAggregateReviewing(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(liveRunDir, "run.json"), []byte(`{"run_id":"`+liveRunID+`","issue":1,"branch":"1-fix"}`), 0644); err != nil {
...
func TestPortal_Compute_ReviewVerdictFromDecisionFileOnDisk(t *testing.T) {
	...
	writePortalLog(t, filepath.Join(repoRoot, ".sandman", "events.jsonl"), []events.Event{
		{Type: "run.started", Timestamp: startedAt.Add(-13 * time.Minute), RunID: implRunID, Issue: 1719, Payload: map[string]any{
			"branch":      "1719-ci-add-macos-latest",
			"batch_id":    implBatchID,
			"base_branch": "main",
		}},

...
func TestStrandedCmd_ListsAllStrandedWorktrees(t *testing.T) {
	...
	out := buf.String()
	if !strings.Contains(out, "2-wrong") {
		t.Errorf("expected output to mention 2-wrong, got: %q", out)
	}
	if !strings.Contains(out, "3-detached") {
		t.Errorf("expected output to mention 3-detached, got: %q", out)
	}
	if !strings.Contains(out, "Run: git -C") {
		t.Errorf("expected remediation command in output, got: %q", out)
	}
	if strings.Contains(out, "1-healthy is on") {
...
func TestStrandedCmd_JSONOutput(t *testing.T) {
	...
	wtPath := filepath.Join(worktreeBase, "9-bad")
	runGit(t, repoDir, "worktree", "add", wtPath, "9-bad")
	...
	if results[0].ExpectedBranch != "refs/heads/9-bad" {
		t.Errorf("ExpectedBranch: got %q, want %q", results[0].ExpectedBranch, "refs/heads/9-bad")
	}
...
func TestStrandedCmd_DefaultsWorktreeDirFromConfig(t *testing.T) {
	...
	runGit(t, repoDir, "branch", "5-stuck")

	worktreeBase := filepath.Join(repoDir, ".sandman", "worktrees")
	if err := os.MkdirAll(worktreeBase, 0755); err != nil {
		t.Fatalf("mkdir worktreeBase: %v", err)
	}
	runGit(t, repoDir, "branch", "1-other")
	addStrandedWorktree(t, repoDir, worktreeBase, "5-stuck", "1-other")
	...
	if !strings.Contains(buf.String(), "5-stuck") {

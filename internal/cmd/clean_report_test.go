...
func TestClean_All_PrintsRemovedReport(t *testing.T) {
	deps := newRunDepsAuto(t, &fakeBatchRunner{})
	dir, err := os.Getwd()
	if err != nil {
		t.Fatalf("os.Getwd: %v", err)
	...
	batchDir := filepath.Join(dir, ".sandman", "batches", "batch-all-report")
	worktreeDir := filepath.Join(dir, ".sandman", "worktrees", "sandman", "all-ac")
	if err := os.MkdirAll(worktreeDir, 0755); err != nil {
		t.Fatalf("create worktree: %v", err)
	}
	writeRunManifest(t, batchDir, batchindex.RunManifest{
		RunID:        "batch-all-report",
		BatchID:      "batch-all-report",
		Issue:        2061,
		Branch:       "sandman/2061-all",
		WorktreePath: worktreeDir,
		Kind:         batchindex.KindIssue,
		Status:       batchindex.RunManifestStatusActive,
	})
	now := time.Now()
	writeBatchIndex(t, dir, []batchindex.Batch{
		{ID: "batch-all-report", Path: batchDir, Kind: batchindex.KindIssue, Status: batchindex.StatusArchived, CreatedAt: now},
	})
	...
	out := buf.String()
	if !strings.Contains(out, "Removed 1 batch entries:") {
		t.Errorf("expected 'Removed 1 batch entries:' header from --all, got: %s", out)
	}
	if !strings.Contains(out, "batch-all-report") {
		t.Errorf("expected per-line to include batch ID under --all, got: %s", out)
	}
}

func TestClean_DryRun_NoBatches_PrintsNothingToRemove(t *testing.T) {
	deps := newRunDepsAuto(t, &fakeBatchRunner{})
	dir, err := os.Getwd()
	if err != nil {
		t.Fatalf("os.Getwd: %v", err)
	...
	if !strings.Contains(out, "Nothing to remove.") {
		t.Errorf("expected 'Nothing to remove.' message on dry-run with no batches, got: %s", out)
	}
	if strings.Contains(out, "No batches to clean.") {
		t.Errorf("stale 'No batches to clean.' should no longer appear, got: %s", out)
	}
	if strings.Contains(out, "Would remove") {
		t.Errorf("dry-run with no batches should NOT print 'Would remove' header, got: %s", out)
	}
}

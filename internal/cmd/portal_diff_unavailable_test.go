...
// TestPortalDiffCreateRunRow_UnavailableRowGetsRowClass covers slice
// #2 of #1312: a row whose Unavailable flag is true renders with the
// `row-unavailable` CSS class on the <tr>. The badge itself keeps
// its status-derived class (success/failure/etc) per the existing
// portal convention; archived/unavailable state is shown via the
// row-level mute class, not a new badge class.
func TestPortalDiffCreateRunRow_UnavailableRowGetsRowClass(t *testing.T) {
...
if (!badge.classList.contains('success')) {
  throw new Error('expected badge to keep status-derived class "success", got ' + badge.className);
}
console.log('PASS');
`
	runNodeScript(t, js)
...
// to render with row-archived + their original status badge class.
...
if (row.classList.contains('row-unavailable')) {
  throw new Error('archived-only row must not carry row-unavailable');
}
const badgeCell = row.querySelector('[data-cell="badge"]');
const badge = badgeCell.children[0];
if (!badge.classList.contains('success')) {

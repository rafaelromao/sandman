...
func TestPortalRunsIndex_ReadEvents_AppendsJSONLTail(t *testing.T) {
	...
	writePortalLog(t, logPath, []events.Event{{Type: "run.started", Timestamp: firstTS, RunID: "abcd-260618113825-42", Issue: 42, Payload: map[string]any{"branch": "sandman/42-fix"}}})
	...
	writePortalLog(t, logPath, []events.Event{{Type: "run.finished", Timestamp: secondTS, RunID: "abcd-260618113825-42", Issue: 42, Payload: map[string]any{"status": "success", "branch": "sandman/42-fix"}}})
...
func TestPortalRunForKey_UsesSharedRunsIndex(t *testing.T) {
	...
	idx.snapshotCache = []portalRun{{Key: "abcd-260618113825-1", RunID: "abcd-260618113825-1", Kind: "completed", Status: "success", IssueLabel: "#1"}}
	idx.mu.Unlock()

	run, err := portalRunForKey(repoRoot, "abcd-260618113825-1")
	if err != nil {
		t.Fatalf("portalRunForKey: %v", err)
	}
	if run.Key != "abcd-260618113825-1" || run.RunID != "abcd-260618113825-1" {
...
func TestPortalHandler_RunsServesSharedRunsIndexSnapshot(t *testing.T) {
	...
	idx.mu.Lock()
	idx.snapshotAt = time.Now()
	idx.snapshotCache = []portalRun{{Key: "abcd-260618113825-99", RunID: "abcd-260618113825-99", Kind: "completed", Status: "success", IssueLabel: "#99"}}
	idx.mu.Unlock()
	...
	if runs[0].Key != "abcd-260618113825-99" {
...
func TestPortalRunsIndex_DiscoverActiveRuns_RefreshesManifestCacheOnChange(t *testing.T) {
	...
	runDir := filepath.Join(repoRoot, ".sandman", "batches", "abcd-260618113825-1")

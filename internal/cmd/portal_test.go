...
func TestPortal_RunFromActiveMatchReturnsReviewingForPRInstance(t *testing.T) {
	...
	if run.IssueLabel != "PR 42" {
		...
		// label uses the "PR <n>" convention (issue #2296/#1667).
...
func TestPortal_RunFromState_ActiveFreshBatchCarriesBatchKey(t *testing.T) {
	...
	run := (&portalRunsView{}).runFromState(repoRoot, state, &active, nil, nil, nil)
...
func TestPortal_ReviewRunLifecycle(t *testing.T) {
	t.Run("active socket shows reviewing", func(t *testing.T) {
		...
		if got.IssueLabel != "PR 42" {
			...
			// uses the "PR <n>" convention (issue #2296/#1667).
			t.Fatalf("expected IssueLabel 'PR 42', got %q", got.IssueLabel)
		}
	...
	t.Run("dead socket after restart", func(t *testing.T) {
		...
		if got.IssueLabel != "PR 42" {
			t.Fatalf("expected IssueLabel 'PR 42' on completed review run, got %q", got.IssueLabel)
		}
		if got.Reason != "review" {
	...
	t.Run("event log only keeps review metadata", func(t *testing.T) {
		...
		if got.IssueLabel != "PR 42" {
			t.Fatalf("expected IssueLabel 'PR 42' for event-log-only review run, got %q", got.IssueLabel)
		}
		if got.Reason != "review" {
...
func TestPortal_ReasonField_PopulatedFromRunKind(t *testing.T) {
	...
	t.Run("in-flight review run", func(t *testing.T) {
		...
		run := (&portalRunsView{}).runFromState(root, state, nil, nil, nil, nil)
	...
	t.Run("aborted review run", func(t *testing.T) {
		...
		if run.IssueLabel != "PR 42" {
			t.Fatalf("expected IssueLabel 'PR 42' on aborted review run, got %q", run.IssueLabel)
		}
	})

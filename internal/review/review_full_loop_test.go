...
func TestReviewDaemon_FullLoopPastLaunchReview(t *testing.T) {
	...
	// registers the trigger as pending. tickAndWait is the macOS-CI-
	// blessed helper from daemon_test.go: it calls d.tick(ctx), then
	// d.WaitForIdle(ctx) to absorb the goroutine launch race on
	// heavily loaded macOS GitHub Actions runners.

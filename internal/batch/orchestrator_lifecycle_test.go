...
func TestDecideImplementationPRLifecycle_NonResolvedGates(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			wantGate: lifecycleGate("CI_FAILURE"), wantAction: lifecycleResume,
		},
		{
			...
			wantGate: lifecycleGate(actionableFeedbackReason), wantAction: lifecycleResume,
		},
		{
			name: "ready to merge",
			pr: &github.PR{Number: 42, State: "open", MergeStateStatus: "CLEAN",
				HeadRefOid: "current-sha"},
			wantGate: lifecycleGate("READY_TO_MERGE"), wantAction: lifecycleResume,
		},
		{
			name: "head drifted",
			pr: &github.PR{Number: 42, State: "open", MergeStateStatus: "CLEAN",
				HeadRefOid: "other-sha"},
			wantGate: lifecycleGate("STALE_PR_HEAD"), wantAction: lifecycleResume,
		},
		{

...
func (o *Orchestrator) validateBatchBranches(req Request) error {
	...
	return fmt.Errorf("refusing to start batch: branches already exist from previous runs: %s. Delete them with git branch -D <branch> or re-run with --override to restart from scratch or --continue to resume", strings.Join(conflicts, ", "))
}

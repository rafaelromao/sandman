...
func (s *runSession) ciWaitEvidence(workDir string, pr *github.PR, headSHA string) (map[string]any, error) {
	...
	if registration.Protocol != ciWaitProtocol || registration.PullRequest != pr.Number || registration.DeadlineUnixSeconds <= registration.StartedUnixSeconds || registration.EffectiveTimeoutSecs <= 0 || registration.DeadlineUnixSeconds-registration.StartedUnixSeconds != registration.EffectiveTimeoutSecs {

...
4. **You must use `codeindex` before `grep` or `glob` when looking for symbols, blast radius, dependencies, or other broad code locations.** If `codeindex.json` exists, run the appropriate `codeindex` command first. Do not start discovery with `grep`, `rg`, or `glob` for that purpose unless `codeindex` cannot answer the question.

...
gh pr list --state merged --repo "$REPO" --search "Closes #<ID> in:body" --json number --jq 'length'
```

- If issue state is `CLOSED` OR a merged PR is found closing this issue → run:

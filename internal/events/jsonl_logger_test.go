...
// would re-warn on the same bytes. Read must
...
func TestJSONLLogger_AppendOnlyRetainsPriorHistory(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "events.jsonl")
	logger := &JSONLLogger{Path: path}

	blocked := Event{Type: "run.blocked", Timestamp: time.Date(2025, 1, 1, 12, 0, 0, 0, time.UTC), RunID: "run-blocked-1", Issue: 42, Payload: map[string]any{"blocked_by": []int{7}}}
	if err := logger.Log(blocked); err != nil {
		t.Fatalf("log blocked: %v", err)
	}
	// Simulate override: no deletion, append new attempt events.
	next := Event{Type: "run.started", Timestamp: time.Date(2025, 1, 2, 12, 0, 0, 0, time.UTC), RunID: "run-42-2", Issue: 42}
	if err := logger.Log(next); err != nil {
		t.Fatalf("log next: %v", err)
	}
	got, err := logger.Read()
	if err != nil {
		t.Fatalf("read: %v", err)
	...
	if len(got) != 2 {
		t.Fatalf("expected 2 events (prior blocked retained), got %d", len(got))
	}
	if got[0].Type != "run.blocked" || got[0].RunID != "run-blocked-1" {
		t.Errorf("first event not retained blocked: %+v", got[0])
	}
	if got[1].RunID != "run-42-2" {
		t.Errorf("second event missing: %+v", got[1])
	}
}

...
Release automation uses this token instead of `GITHUB_TOKEN` because pull requests created with `GITHUB_TOKEN` do not start `pull_request` workflows. The maintainer token lets release-please create or update the release pull request and allows it to receive the required `CI / build` and `CI / semantic-pull-request` checks. The exhaustive Linux regression and the focused `Native Compatibility` workflows (three parallel arch-specific jobs: `darwin/amd64`, `darwin/arm64`, and `linux/arm64`) run separately when Release Please pushes to its release branch; their results are visible on the release pull request and must be reviewed before merging. Never print the token or commit it to the repository. A missing `RELEASE_PLEASE_TOKEN` intentionally fails the release workflow rather than falling back to an unverified credential.
...
- The full regex is documented in [`AGENTS.md`](AGENTS.md#branching-and-versioning-rules). Release Please derives SemVer from merged commit history, not from the title check alone; see the [releasing guide](docs/development/releasing.md#versioning-policy) for the mapping. With the initial RC published, Release Please calculates subsequent RC versions automatically from release-bearing merged history.

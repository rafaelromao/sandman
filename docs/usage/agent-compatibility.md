...
Sandman container images also include `codeindex`. Agents use it to answer repo-structure questions quickly, especially when a task needs symbol lookup or impact analysis across a large codebase. That keeps large source trees navigable without forcing the model to read or summarize every file first.

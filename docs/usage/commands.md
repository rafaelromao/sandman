...
- `run` preserves worktrees for `failure`/`aborted`/`blocked`/`await` (for inspection and `--continue`); succeeded runs are auto-cleaned (worktree + local branch removed) so `sandman clean` is only needed for remaining cases

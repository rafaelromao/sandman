...
- **Build tools preset** — container recipe for the image (generic, dotnet, elixir, go, node, or python)
- **Tool version** — version selector for the build toolchain

Sandman auto-detects repo hints and defaults to the matching BuildToolsPreset when it finds .NET, Elixir, Go, Node, or Python project files; otherwise it falls back to `generic`.

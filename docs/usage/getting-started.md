...
- **Build tools preset** — preset-specific tooling for the container image (generic, go, node, python, or rust)
- **Tool version** — version selector for the build toolchain

Sandman auto-detects repo hints and defaults to the matching BuildToolsPreset when it finds Go, Node, Python, or Rust hints; otherwise it falls back to `generic`.

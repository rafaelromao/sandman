...
- **Build tools preset** — container recipe for the image (generic, dotnet, go, node, python, or elixir)
- **Tool version** — version selector for the build toolchain

Sandman auto-detects repo hints and defaults to the matching BuildToolsPreset when it finds .NET, Go, Node, Python, or Elixir project files; otherwise it falls back to `generic`.

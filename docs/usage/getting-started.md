...
- **Build tools preset** — language-specific tooling for the container image (generic, go, java, node, or python)
- **Tool version** — version selector for the build toolchain

Sandman auto-detects repo hints and defaults to the matching BuildToolsPreset when it finds Go, Java (`pom.xml`, `build.gradle`, or `build.gradle.kts`), Node, or Python project files; otherwise it falls back to `generic`.

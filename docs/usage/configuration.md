...
# Sandman installs the built-in agent in scaffolded Dockerfiles, installs `codeindex` for fast symbol lookup, and mounts the shared skills directory.
```
...
Sandman also installs `codeindex` in scaffolded container images. Sandman runs agents against real repositories, and `codeindex` gives the agent a fast way to find symbols, trace dependencies, and inspect impact without reading large source trees line by line.

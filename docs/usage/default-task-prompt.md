...
- `AFK Rule — Absolute` replaces human approval with subagent consensus for plan approval, and bans subagent use for PR review (must use `sandman-pr-review` skill). Self-review uses `sandman-code-review` skill.
...
- `Required Skill Chain` lists `sandman-implement` first (the parent skill that owns the end-to-end implement workflow and the closing-reference body rule), then enumerates the delegated subskills it calls (`sandman-tdd`, `sandman-code-review`, `sandman-back-merge`, `sandman-pr-review`, `sandman-pr-merge`).

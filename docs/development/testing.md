...
| Linux arm64 | (no CI today; covered only by the release archive) | n/a | n/a |
| macOS amd64 (Intel) | n/a (covered transitively by `CI / build (macos-latest)` arm64 today; future Intel matrix entry tracked separately) | n/a — release-please does not run Podman on macOS after #2459 | `Native Compatibility / (darwin/amd64)` (release-please branch + dispatch) |
| macOS arm64 (Apple Silicon) | `CI / build (macos-latest)` (PR + push to `main`) | n/a | `Native Compatibility / (darwin/arm64)` (release-please branch + dispatch) |
...
- `Native Compatibility` is a focused macOS suite with two parallel jobs — `Native Compatibility (darwin/amd64)` on `macos-15-intel` and `Native Compatibility (darwin/arm64)` on `macos-14`. Each job builds Sandman natively for its arch, then exercises Darwin socket paths plus portal and attach streaming, and verifies the native portal and attach streaming boundary through two hermetic portal E2E cases. Linux-container preset portability remains covered by the Linux workflow rather than being rebuilt inside the macOS Podman VM.

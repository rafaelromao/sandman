# ADR-0020: Remove Pi agent support
...
Over time, maintaining Pi parity became pure overhead with no corresponding user demand. Every feature that touched agent behaviour required a Pi counterpart: config snapshots (ADR-0015/~~ADR-0017 (deleted)~~), test model env vars (ADR-0018), scaffolded Dockerfiles, command templates, and documentation. The cost of carrying Pi-specific branches, test cases, and documentation edits was not offset by organic usage.
...
2. ADR-0006 (built-in agent presets) and ADR-0018 (test model env vars) are left as immutable historical records; this ADR serves as the current decision that supersedes the Pi-specific context in those documents.
3. Mark ~~ADR-0017 (deleted)~~ (Pi config snapshot split) as superseded by this ADR.
...
- The historical record of Pi's existence is preserved in the repository via ~~ADR-0017 (deleted)~~, ADR-0018, and the git history of the files modified by this change.

### Neutral

- ~~ADR-0017 (deleted)~~ remains on disk with a superseded status; readers are directed to this ADR for context on the removal.

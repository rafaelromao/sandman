...
**On-disk claim lock (`TryClaim`):** Before adding eyes reactions and calling `launchReview`, `processPR` calls `ReviewStateStore.TryClaim(commentID)` against a freshly-loaded store for the new run folder. If the claim fails (another tick has the same comment claimed or terminal-seen), `processPR` returns immediately without calling `launchReview` or mutating GitHub. If `launchReview` returns an error and the run folder's `review-state.json` was not written, `processPR` calls `state.Release(commentID)` so the next tick retries.

The claim lock is persisted in the run folder's `review-state.json` so an unexpected daemon restart also honors the in-flight lock on re-scan.

The `busy=1` outer serialization makes the structural race (two ticks in `processPR` for the same PR) impossible. The `TryClaim` on-disk lock is defense-in-depth: it ensures a stale in-memory claim from a crashed tick is honored even after process restart, and it handles the edge case where two ticks enter `processPR` for the same PR through the trigger channel used in tests.

...
Add a new `sandman portal` command that binds a local HTTP server to `0.0.0.0`, defaults to port `5000`, discovers run sockets by scanning the current repository's `.sandman/runs/` tree on every request, and exposes a `/api/commands` preset launcher surface for repo-scoped Sandman commands.

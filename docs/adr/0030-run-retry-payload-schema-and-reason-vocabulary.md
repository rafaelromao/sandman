...
- **Active run** — no `run.finished` exists yet. Walk the raw event list for the run and return the count of retries that have actually occurred (initial run excluded), which equals `max(attempt - 1)` across the `run.retry` events, with each candidate clamped at 0 so a malformed payload cannot produce a negative count. Each `run.retry` event's `attempt` field is the 1-indexed about-to-start attempt number, so the orchestrator's first retry emits `attempt: 2` (corresponding to one retry), the second retry emits `attempt: 3` (corresponding to two retries), and so on. If no `run.retry` events exist, return `0` (the run has not retried yet). Slice 1's `LiveAttempt()` helper on `events.RunState` is the canonical implementation; slice 2's `Retries []Event` projection makes that walk cheap by avoiding a second pass over the events list. The parallel state-absent path in `internal/cmd/portal_runs_view.go::attemptsAndLastRetryReasonFromEvents` follows the same rule.

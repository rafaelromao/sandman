...
- ADR-0008 already foreshadowed this trade-off in its "Negative" section: "users with multi-GB config trees may want to exclude cache-like subdirectories from `config_dirs`." This ADR provides the explicit mechanism that recommendation needed. ADR-0008's "Negative" wording is left as-is; the new `SnapshotExcludes` mechanism is the implementation of that recommendation rather than a contradiction of it.

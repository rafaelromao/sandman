...
- **Record site (primary)**: `Daemon.processPR` records every non-trigger comment it observes into `SelfPostStore.Record` after the `ParseTrigger` check (the trigger itself is NOT recorded — see below). On the next tick, the same comment's hash is already in the store and `IsSelfPosted` drops it before `ParseTrigger`. This is the sole authoritative record site (see the ownership note below for the rationale). The bot's review-body, by contrast, can inadvertently contain trigger-substring text (PR #1671) and MUST be filtered to break the self-re-trigger loop.

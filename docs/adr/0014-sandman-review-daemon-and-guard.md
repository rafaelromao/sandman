...
- **Record site (primary)**: `Daemon.promotePendingComment` records any non-trigger comment it observes after `since`, so every bot-posted review body leaves a hash for the next tick. This is the sole authoritative record site — the skill side no longer maintains `.sandman/reviews/self-posted.json` from any wrapper (see the ownership note below). Only bot-posted bodies are recorded; the implementor's trigger command is intentionally NOT recorded. The trigger is a request for review, not a bot-comment to filter — tracking its hash would mask future legitimate trigger lookups. The bot's review-body, by contrast, can inadvertently contain trigger-substring text (PR #1671) and MUST be filtered to break the self-re-trigger loop.
- **Filter site**: `Daemon.processPR` calls `selfPosts.IsSelfPosted(comment.Body)` on each PR comment BEFORE `ParseTrigger` (issue #1702 reverses the #1682 ordering, which had the filter apply only to non-trigger comments). A self-posted comment is dropped from the trigger pipeline and is not added to `MarkSeen`. Because the SelfPostStore only ever contains bot-posted bodies (review bodies, never the implementor's trigger command), applying the filter before ParseTrigger does not regress trigger detection for fresh human-issued triggers; it only drops the bot's own review body before its body is parsed for the trigger substring. This is what breaks the self-loop in PR #1671 — the bot's review body containing the literal `/sandman review` substring in its `## Previous review progress` section would otherwise re-trigger on a later tick.

### Ownership note (issue #1757)

The earlier design split the SelfPostStore's record site between the `pr-review` SKILL.md Step 4b wrapper (primary, ran inside the LLM agent after `gh pr comment` returned) and `Daemon.promotePendingComment` (defensive observation in the daemon). That split was the structural source of the cross-PR poisoning observed on PR 1752: the skill-side wrapper required the LLM to compute a SHA-256 of the body it just posted and write it into the daemon's state file, putting a responsibility-boundary violation in place where the daemon relied on an LLM reading its instructions correctly to maintain its own state. As of issue #1757, the skill no longer maintains `.sandman/reviews/self-posted.json` from any wrapper; `Daemon.promotePendingComment` is the sole authoritative record of bot-posted bodies. The review prompt's hard rule that forbids emitting the literal `{{REVIEW_COMMAND}}` substring in the review body remains as defence-in-depth, but it is no longer load-bearing — the daemon-side filter breaks the self-loop regardless of whether the bot ever emits the substring.

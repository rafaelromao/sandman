...
- The snapshot lives under `.sandman/runs/<run-id>/config/` and is cleaned up when the run dir is removed, regardless of success or failure. See ADR-0015 for the move out of `/tmp` and the stale-snapshot sweep added to `sandman run`, `sandman continue`, and `sandman clean`.

...
2. **Detach HEAD in the main repo and drop the stray ref.** If the main repo itself is checked out on `sandman/N-…`, the helper runs `git -C <repoRoot> checkout --detach` followed by `git -C <repoRoot> update-ref -d refs/heads/<branch>`. The detach leaves the working tree contents unchanged at the same commit (so the operator's editor / uncommitted changes are preserved); the ref-drop releases the branch so the worktree registration can be removed and re-created. The branch reference is re-created later in the run by the canonical `git worktree add -b <branch>` call, so the effective loss is bounded to a transient detached-HEAD window in the parent repo.
...
All four strategies are local to the worktree base and the main repo. They never touch refs that the operator has not touched in this session, and they never force-push. The ref-drop in strategy (2) is bounded to a single Start() invocation; the branch is re-created immediately after by the canonical `git worktree add -b <branch>` call downstream.
...
- Strategy (2)'s `git update-ref -d` is destructive in the sense that it drops a ref. The branch reference is re-created by `git worktree add -b <branch>` immediately after, so the effective loss is bounded to a transient detached-HEAD window. If `Start` fails between the ref-drop and the re-creation, the operator is left with a detached HEAD on a deleted branch ref; the reflog preserves the commit so `git checkout -b <branch> <reflog-sha>` recovers.

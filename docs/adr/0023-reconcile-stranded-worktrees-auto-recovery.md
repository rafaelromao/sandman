...
The recovery loop tries five strategies, in order, and stops as soon as one succeeds:
...
3. **Detach HEAD in the main repo and delete the branch.** If the main repo itself is checked out on `sandman/N-…` (or the post-ADR-0040 unprefixed `<n>-<slug>`), the helper runs `git -C <repoRoot> checkout --detach` followed by `git -C <repoRoot> branch -D <branch>`. The detach leaves the working tree contents unchanged at the same commit (so the operator's editor / uncommitted changes are preserved); `git branch -D` re-checks worktree holders atomically with the delete — closing the TOCTOU window where a sibling worktree could check out the branch between the parent's detach and the delete. The branch is re-used later in the run by the worktree-add step (`git worktree add -f <path> <branch>`), so the effective loss is bounded to a transient detached-HEAD window in the parent repo.
...
All five strategies are local to the worktree base and the main repo. They never touch refs that the operator has not touched in this session, and they never force-push. The ref-drop in strategy (3) is bounded to a single Start() invocation; the branch is re-used immediately after by the worktree-add step downstream.

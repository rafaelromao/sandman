...
- **Review without a linked issue:** `<shortid>-<ts>-PR<pr>` (subject is `PR<pr>`).
- **Review with a linked issue:** `<shortid>-<ts>-<linkedIssue>-PR<pr>` (subject is `<linkedIssue>-PR<pr>`).

The `<shortid>-<ts>` prefix is owned by `runid.NewBatch()`; the per-row subject is composed by `internal/review.reviewRunIDFor`. The legacy literal `RunID: "review"` alias and the `runs/review/` folder name are not canonical — older review launches predating issue #1551 used them as both the row RunID and the run folder name, but every review run now lands under a folder whose name is exactly its per-row RunID. The `<shortid>-<ts>-<linkedIssue?>-PR<pr>` shape is the only source of truth for review run identity. References ADR-0030 and ADR-0034.
_Avoid_: review-only RunID, special review alias, `runs/review`.
_See_: Run, RunID, Review daemon state.

**Review daemon state**:
Flat files under `.sandman/reviews/` for daemon-level state only: `review.sock` (daemon command socket), `review-prompt.md` (shared prompt template), and `self-posted.json` (issue #1648, post-#1756: a JSON object keyed by composite `pr-<N>-<sha>` strings, where `sha` is hex(`sha256(body)`), scoped per PR number; the dedup key is `(prNumber, sha256(body))` so the bot's own review-comment cannot be mistaken for a fresh `/sandman review` trigger on a later tick). The daemon's `SelfPostStore` is in-memory `map[prNumber]map[sha]selfPostEntry` (see `SelfPostStore`). The loader is layered (issue #1821): missing files start empty with no rename; new-shape files (every key matches `pr-<N>-<sha>`) are loaded into memory, preserving prior-session entries; legacy / mixed / corrupt files fall through to the issue #1756 one-shot rename + greenfield path (because a silent-merge on those shapes would either drop bodies or mis-attribute them). On startup, `Daemon.New` also greps every review-kind batch's `runs/<rowID>/run.log` via `extractBodiesFromLog` and records every extracted body via `Record`, closing the across-restart failure mode observed on PR #1809 even for operators with a greenfield or legacy-shape store. The daemon records bot-authored bodies into `SelfPostStore` via `Daemon.promotePendingComment` on a per-tick basis (issue #1759), gated by `extractBodiesFromLog` greppping the bot's own review run log (see `Review run log`) for `gh pr comment <N> --body <body>` invocations; the `pr-review` skill no longer maintains the store from any wrapper (issue #1757). The folder holds **no** per-PR subdirectories and **no** per-row RunID folders — per-run review state (`review-state.json` with seen comments and claim locks) lives inside the batch run folder at `.sandman/batches/<batch-id>/runs/<runID>/review-state.json`, where `<runID>` is the canonical per-row RunID for the review run (see `Review run`), NOT the legacy `runs/review` alias. Dedup key is `(prNumber, commentID)`. The daemon additionally maintains a per-process in-memory `seenCache` (`prNumber → map[commentID]bool`) hydrated at construction from the on-disk batches index and refreshed on every successful `ReviewStateStore.MarkSeen` (status ∈ `{success, superseded}`); the cache is advisory and must be invalidated on every `MarkSeen` so the per-tick scan can short-circuit without re-reading `.sandman/batches.json` or any `review-state.json`. Under the lazy-verify con... (line truncated to 2000 chars)
_Avoid_: review state, PR state.

**SelfPostStore**:
The review daemon's in-memory + on-disk store of bot-posted review bodies. The in-memory shape is `map[prNumber]map[sha]selfPostEntry` (per-PR outer map, hex sha256 inner map) and the on-disk shape is a single JSON object at `.sandman/reviews/self-posted.json` keyed by the composite `pr-<N>-<sha>` string, persisted atomically via temp-file + `os.Rename`. The dedup key is `(prNumber, sha256(body))` — per-PR scoping closes the cross-PR poisoning failure mode observed on PR 1752 (issue #1756). The lookup contract is `IsSelfPosted(prNumber, body) bool`; nothing else. The write path is daemon-owned: `Daemon.processPR`'s review path owns the store end-to-end, and the canonical `Record` call lives inside `Daemon.promotePendingComment` (per-tick lazy-verify step) and `Daemon.New` (cross-restart seed step), both gated by `extractBodiesFromLog` greppping the bot's own review run log (see `Review run log`) to confirm the body came from the bot running in the daemon's launched subprocess (issue #1759; seed step added in issue #1821). The `pr-review` skill does not touch the store — the LLM agent is not in the loop on the write path (issue #1757). The layered loader (issue #1821): missing files start empty with no rename; new-shape files (every key matches `pr-<N>-<sha>`) are loaded into memory, preserving prior-session entries across restarts; legacy / mixed / corrupt files fall through to the issue #1756 one-shot rename + greenfield path because a silent-merge on those shapes would either drop bodies or mis-attribute them. The bot's own review run logs are the only durable source for what the bot posted (the daemon greps them via the seed step so operators with a greenfield or legacy-store deployment also recover across-restart memory). References ADR-0014 §Self-posted comment filter, §Loader behavior, §Cross-restart seeding, §Structural self-defence sniff, §Discovery via run-log grep.
_Avoid_: self-posted store, dedup store, posted bodies store.
...
**Review run log**:
The same on-disk file as the **Saved Run Log** (`.sandman/batches/<batch-id>/runs/<run-id>/run.log`), viewed from the review daemon's perspective as the authoritative source for which bodies the bot posted during a review run. The daemon's helper `extractBodiesFromLog(runLogPath)` greps the log for `gh pr comment <N> --body <body>` invocations (and `--body-file` variants) and returns every body the bot posted, in chronological order. The daemon is the only entity that both sees the review run log and can decide which bodies are self-authored, so the run log is the right ownership boundary for discovering bot-authored bodies (issue #1759). The review daemon records those bodies into `SelfPostStore` so the next tick's `IsSelfPosted`-first filter drops them before parsing them for the trigger substring. References ADR-0014 §Self-posted comment filter.
_Avoid_: bot log, agent log, comment log.
_See_: Saved Run Log, SelfPostStore.

...
The component that detects Specification issues, discovers their child issues (from `#N` shorthand or full issue URLs in the body and comments, native GitHub sub-issues, and a fallback mention search), verifies each candidate by its `## Parent` backlink, and replaces the Specification with its accepted children in the input batch. Runs in `sandman run` after issue selection and before `DependencyResolver.Resolve`. A Specification with no accepted children runs as a single-row batch, logged as `running issue #N as a regular issue (no children)` per ADR-0040; a child that is itself a Specification is expanded recursively (per-flatten log line) rather than rejected, with depth bounded by the unique-set seen+addUnique pair. Candidates that are also in the user-typed input list bypass the `## Parent` verification (the user owns the choice), as documented in ADR-0025 §3a. The carve-out for the recursive path pulls the ancestor user input back into the output via the per-flatten expansion. References ADR-0025 §4 for the destination-aligned recursive-flatten invariant.

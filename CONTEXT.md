...
**RunDir**:
A per-batch directory under `.sandman/runs/` named by the `<shortid>-<ts>` prefix and a kind-specific suffix. Generated by `internal/runid.NewBatchID`. Contains the control socket, command server, broadcaster, and `batch.json` manifest for the daemon process. References ADR-0030.

...
**RunDir**:
The `<batch>/` directory that is the daemon process's on-disk home, named by the `<shortid>-<ts>` prefix and a kind-specific suffix (generated by `internal/runid.NewBatchID`). Contains `batch.json` and `batch.sock`. Each AgentRun within the batch gets its own `<batch>/runs/<runID>/` subdirectory containing `run.json`, `run.log`, and `run.sock`. References ADR-0030.
...
- The batch directory (`<batch>/`) is the **Daemon Process**'s home; once it has been moved to `.sandman/archive/<run-id>` the directory is no longer owned by any **Daemon Process**
- A **RunDir** is generated by `NewBatchID` for each **Batch** and contains the daemon's `batch.json`, `batch.sock`, and per-row `<runID>/` subdirectories

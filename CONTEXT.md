...
**Review daemon state**:
Flat files under `.sandman/reviews/` for daemon-level state only: `review.sock` (daemon command socket), `review-prompt.md` (shared prompt template), and `self-posted.json` (issue #1648, post-#1756: a JSON object keyed by composite `pr-<N>-<sha>` strings, where `sha` is hex(`sha256(body)`), scoped per PR number; the dedup key is `(prNumber, sha256(body))` so the bot's own review-comment cannot be mistaken for a fresh `/sandman review` trigger on a later tick). The daemon's `SelfPostStore` is in-memory `map[prNumber]map[sha]selfPostEntry` (see `SelfPostStore`). The loader is greenfield: any pre-existing file at startup is atomically renamed to `self-posted.json.ignore-<ts>.bak` and the store starts empty (issue #1756 closes the cross-PR poisoning failure mode from PR 1752). The daemon records bot-authored bodies into `SelfPostStore` via `Daemon.promotePendingComment`, gated by `extractBodiesFromLog` greppping the bot's own review run log (see `Review run log`) for `gh pr comment <N> --body <body>` invocations (issue #1759); the `pr-review` skill no longer maintains the store from any wrapper (issue #1757). The folder holds **no** per-PR subdirectories and **no** per-row RunID folders — per-run review state (`review-state.json` with seen comments and claim locks) lives inside the batch run folder at `.sandman/batches/<batch-id>/runs/<runID>/review-state.json`, where `<runID>` is the canonical per-row RunID for the review run (see `Review run`), NOT the legacy `runs/review` alias. Dedup key is `(prNumber, commentID)`. The daemon additionally maintains a per-process in-memory `seenCache` (`prNumber → map[commentID]bool`) hydrated at construction from the on-disk batches index and refreshed on every successful `ReviewStateStore.MarkSeen` (status ∈ `{success, superseded}`); the cache is advisory and must be invalidated on every `MarkSeen` so the per-tick scan can short-circuit without re-reading `.sandman/batches.json` or any `review-state.json`. Under the lazy-verify contract (issue #1482 slice D), the daemon adds a third per-process in-memory structure, `pendingReviews` (`prNumber → []... (line truncated to 2000 chars)
_Avoid_: review state, PR state.

**SelfPostStore**:
The review daemon's in-memory + on-disk store of bot-posted review bodies. The in-memory shape is `map[prNumber]map[sha]selfPostEntry` (per-PR outer map, hex sha256 inner map) and the on-disk shape is a single JSON object at `.sandman/reviews/self-posted.json` keyed by the composite `pr-<N>-<sha>` string, persisted atomically via temp-file + `os.Rename`. The dedup key is `(prNumber, sha256(body))` — per-PR scoping closes the cross-PR poisoning failure mode observed on PR 1752 (issue #1756). The lookup contract is `IsSelfPosted(prNumber, body) bool`; nothing else. The write path is daemon-owned: `Daemon.processPR`'s review path owns the store end-to-end, and the canonical `Record` call lives inside `Daemon.promotePendingComment`, fired only after `extractBodiesFromLog` greps the bot's own review run log (see `Review run log`) and confirms the body came from the bot running in the daemon's launched subprocess (issue #1759). The `pr-review` skill does not touch the store — the LLM agent is not in the loop on the write path (issue #1757). The loader is greenfield: any pre-existing `self-posted.json` at startup is atomically renamed to `self-posted.json.ignore-<ts>.bak` and the store starts empty. References ADR-0014 §Self-posted comment filter.

...
**Archive**:
The on-disk resting place for completed batch directories at `.sandman/archive/<batch-id>/`, mirroring the `batches/` structure 1:1. Populated by `sandman archive run <run-id>` (which also accepts `batch <id>` as an alias) or by `sandman archive older-than <days>` for bulk archival of every dead batch whose manifest `CreatedAt` (or directory mtime when the manifest is missing) is older than the given cutoff. Archiving relocates the batch folder from `.sandman/batches/<batch-id>/` (its live-and-during-run home) to `.sandman/archive/<batch-id>/` so the batches directory stays scoped to currently-relevant batches. The daemon is forbidden from writing to an archived batch; the batch is treated as read-only historical state once moved. References ADR-0032.
_Avoid_: trash, graveyard, old runs, retired runs.

**RunDir**:
Deprecated alias for `BatchDir`. Kept for backward compatibility during the transition period. New code should use `BatchDir`. See ADR-0032.
_Avoid_: run directory, batch dir.
...
**Control Socket**:
A Unix domain socket at `.sandman/batches/<batch-id>/batch.sock` that accepts attach client connections and is used for batch-level liveness checks (`IsRunActive`). Created when a daemon starts, removed when it stops. Distinct from the per-run command socket (`run.sock`). References ADR-0033.
...
**Command Server**:
A Unix domain socket at `.sandman/batches/<batch-id>/run.sock` (batch root) that accepts one-shot JSON command requests from outside the daemon process. First supported command is `{"action":"abort","issue":<n>}`, dispatched to the orchestrator's per-issue cancel API. One command server per batch, dispatching by issue number via the `IssueCommander` seam. Created when a daemon starts, removed when it stops. Distinct from the **Control Socket** (`batch.sock`), which streams daemon output to **Attach** clients. References ADR-0033.
...
**Portal**:
A repo-scoped local HTTP dashboard started by `sandman portal` that reads `.sandman/batches.json` first and only probes the filesystem for live-socket state. Shows active and recent Sandman instances. It does not manage daemon lifecycle. (Note: the preset launcher was removed in #1204.) References ADR-0032.
...
- A **Daemon Process** is created by `sandman run`, which starts a **Control Socket** (`batch.sock`) and a **Command Server** (`run.sock`) at the batch root
- A **Control Socket** at `.sandman/batches/<batch-id>/batch.sock` accepts **Attach** connections for the duration of the **Batch**
- A **Daemon Process** also starts a **Command Server** at `.sandman/batches/<batch-id>/run.sock` for surgical per-issue control requests
...
- A **Portal** reads `.sandman/batches.json` on each poll so newly started **Daemon Process** instances appear without restarting the portal
- An **Archive** entry under `.sandman/archive/<batch-id>` is the relocated home of a batch directory whose **Daemon Process** is no longer live; `sandman archive run <run-id>` performs the move after the liveness check fails
- The batch directory under `.sandman/batches/<batch-id>` is the **Daemon Process**'s home; once it has been moved to `.sandman/archive/<batch-id>` the directory is no longer owned by any **Daemon Process**
- A **Run** folder under `.sandman/batches/<batch-id>/runs/<run-id>/` holds per-run artifacts: `run.json`, `run.log`, `run.sock`
